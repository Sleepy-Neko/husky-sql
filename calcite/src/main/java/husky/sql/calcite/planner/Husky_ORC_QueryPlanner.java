...
//                            "Please check the documentation for the set of currently supported SQL features.");
    ...
    private JSONObject get_condition(List<String> exprList, String expr) {
        ...
        while (itr.hasMoreElements()) {
            ...
            if (new_l.contains("$t"))
                ...
                left_info.put("num of operand", 1);
    ...
    private void generate_json(CalciteConnection conn, RelNode logicalPlan) {
        //Try to find TableScan
        RelOptTable logicalTable = null;
        if (!(logicalPlan instanceof HuskyLogicalNativeTableScan)) {
            RelNode itr_node = logicalPlan;
            while(itr_node.getInputs().size() > 0) {
                    itr_node = itr_node.getInput(0);
                    if(itr_node instanceof HuskyLogicalNativeTableScan)
        ...
        while(logicalPlan.getInputs().size() > 0) {

            if (logicalPlan instanceof HuskyLogicalAgg) {
                ImmutableBitSet groupSet = ((HuskyLogicalAgg) logicalPlan).getGroupSet();
                JSONObject agg = new JSONObject();
                JSONArray groupByList = new JSONArray();
                JSONArray funcList = new JSONArray();
                JSONArray argsList = new JSONArray();

                String groupSet_str = groupSet.toString();
                StringTokenizer itr = new StringTokenizer(groupSet_str, "{}, \t");
                agg.put("listSize", itr.countTokens());

                while (itr.hasMoreElements()) {
                   groupByList.add(Integer.parseInt(itr.nextToken()));
                }
                agg.put("groupByList", groupByList);

                List<AggregateCall> aggCalls = ((HuskyLogicalAgg) logicalPlan).getAggCallList();
                agg.put("funcSize", aggCalls.size());

                for (AggregateCall aggCall : aggCalls) {
                    if (aggCall.toString().startsWith("COUNT"))
                        funcList.add("COUNT");
                    else if (aggCall.toString().startsWith("AVG"))
                        funcList.add("AVG");
                    else if (aggCall.toString().startsWith("SUM"))
                        funcList.add("SUM");
                ...
                agg.put("funcList", funcList);
                agg.put("argList", argsList);
                obj.put("Aggregation", agg);
            }
            ...
            if (logicalPlan instanceof HuskyLogicalCalc) {
                ...
                for (RexNode n : huskyRexProgram.getProjectList()) {
                    String nString = n.toString();
                    int id = Integer.parseInt(nString.substring(2));
                    nString = exprList.get(id);
                    JSONObject elem = new JSONObject();
                    elem.put("name", nString);
                    elem.put("id", id);
